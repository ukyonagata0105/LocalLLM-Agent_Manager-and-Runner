...
ipcMain.handle('sync-llm-config', async (_event, config: {
    ...
    if (config.apiKey) store.set('llm.apiKey', config.apiKey);
    if (config.baseUrl) store.set('llm.baseUrl', config.baseUrl);

    const results: { openhands: boolean; n8n: boolean; dify: boolean } = { openhands: false, n8n: false, dify: false };
...
ipcMain.handle('get-llm-models', async () => {
    ...
    const baseUrl = store.get('llm.baseUrl') as string || 'http://localhost:1234/v1';
    const provider = store.get('llm.provider') as string;
...
import { getRemoteAccessManager, TunnelStatus } from '../src/modules/M11_remote/index';
...
if (storedAuth) {
    authService.importConfig(storedAuth as any);
}
...
ipcMain.handle('start-all-services', async () => {
    const config = {
        apiKey: store.get('llm.apiKey') as string,
        model: store.get('llm.model') as string,
        baseUrl: store.get('llm.baseUrl') as string,
        provider: store.get('llm.provider') as string,
        language: store.get('app.llm_language') as string || 'en',
    };
...
ipcMain.handle('list-files-recursive', async (_event, dirPath: string) => {
    ...
    function walkDir(dir: string) {
        ...
        if (!isPathSafe(dir, projectRoot)) return;
