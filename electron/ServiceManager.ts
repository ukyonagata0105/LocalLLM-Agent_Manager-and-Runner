...
export interface ServiceConfig {
    ...
    baseUrl?: string;
    provider?: string;
    language?: string; // Language code: en, ja, es, zh, ko
}

// Language code to locale mapping for different services
const LANGUAGE_LOCALES: Record<string, { n8n: string; timezone: string; langName: string }> = {
...
class ServiceManager {
     ...
     */
    async waitForDocker(maxWaitMs: number = 30000): Promise<boolean> {
     ...
     */
    async startOpenHands(config?: ServiceConfig): Promise<{ success: boolean; error?: string }> {
        try {
...
`;
     ...
     */
    async startN8n(config?: ServiceConfig): Promise<{ success: boolean; error?: string }> {
        try {
            // Clean up any existing container
            try {
     ...
     */
    async startDify(config?: ServiceConfig): Promise<{ success: boolean; error?: string }> {
...
`;
                    ...
                    if (!envContent.includes('# LLM Configuration (Auto-synced)')) {
                        ...
                        envContent = envContent.replace(
                            /# LLM Configuration \(Auto-synced\)[\s\S]*?(?=\n[A-Z]|\n$|$)/,
                            llmSettings.trim()
                        );
                    }
                }
     ...
     */
    async autoCreateDifyAdmin(): Promise<void> {
     ...
     * Start all services
     */
    async startAllServices(config?: ServiceConfig): Promise<{
        openhands: { success: boolean; error?: string };
        n8n: { success: boolean; error?: string };
        dify: { success: boolean; error?: string };
    }> {
        if (this.isStarting) {
            return {
        ...
        try {
            ...
            const results = {
                openhands: { success: true, error: undefined } as { success: boolean; error?: string },
                n8n: { success: true, error: undefined } as { success: boolean; error?: string },
                dify: { success: true, error: undefined } as { success: boolean; error?: string }
            };
